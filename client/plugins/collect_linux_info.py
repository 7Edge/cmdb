...
def collect():
    ...
    completed_p = subprocess.run(['dmidecode', '-t', 'system'], stdout=subprocess.PIPE)

    sys_info = completed_p.stdout.decode()

    for key in filter_keys:
        key_pattern = r"%s:\s+(?P<%s>.+)\n" % (key, 'value')
        m_obj = re.search(key_pattern, sys_info)
        if not m_obj:
            raw_data[key] = ''
            continue
        raw_data[key] = m_obj.group('value')
